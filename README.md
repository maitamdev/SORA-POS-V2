...
4. Chạy tiếp `database/app_settings.sql`, `database/hardening.sql`, `database/enterprise_pos_core.sql`, `database/stock_atomic_rpc.sql`, và `database/employee_notification_email_and_shift_duration.sql` để bật cấu hình vận hành, ràng buộc dữ liệu, transaction checkout/cancel, transaction kho, email lịch ca và tổng giờ làm.
