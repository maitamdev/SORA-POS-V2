...
const StockPage = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [inventoryRes, alertsRes, transactionsRes] = await Promise.all([
        stockAPI.inventory({ limit: 100 }),
        stockAPI.alerts({ limit: 50 }),
        stockAPI.transactions({ limit: 50 }),
      ]);
    ...
    <div className="space-y-6 animate-fadeIn">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between border-b border-slate-200 pb-5">
        <div>
          ...
          <p className="text-xs sm:text-sm font-medium text-slate-500">Theo dõi tồn kho, cảnh báo và lịch sử giao dịch realtime.</p>
        </div>
          ...
          <label className="block">
            ...
            <select name="product_id" required className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold outline-none">
              ...
              {inventory.map((product) => (
                <option key={product.id} value={product.id}>{product.sku} - {product.name} (tồn {product.stock_quantity})</option>
              ))}
            </select>
          </label>
